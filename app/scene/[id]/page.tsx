...
export default function ScenePage({
  ...
  if (!scenario) {
    return (
      <div
        ...
        style={{ background: "#1a1008", color: "#f59e0b" }}
      >
        <div className="text-center">
          <p className="text-xl mb-4">场景不存在</p>
          <button
            onClick={() => router.push("/")}
  ...
  async function handleSend(input: string) {
    if (input.startsWith("@教练")) {
      ...
      await runCoach("", dialogMessages, true, question)
      return
    }
    ...
    try {
      const charLine = await streamResponse(
        "/api/chat",
        {
          ...
          messages: newDialog.filter((m) => m.role !== "coach"),
          userInput: input,
        },
        setCharStreamText
      )
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-bold" style={{ color: "#f59e0b", fontFamily: "serif" }}>
          {scenario.titleJa}
        </span>
        <span className="text-sm" style={{ color: "#7a5c38" }}>
          {scenario.title}
        </span>
